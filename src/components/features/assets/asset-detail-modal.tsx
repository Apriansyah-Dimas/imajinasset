...
import ImageUpload from "@/components/features/assets/image-upload";
...
const mapSessionEntryToAsset = (entry: SessionEntry): Asset => {
  ...
  const resolvedPurchaseDate =
    entry.tempPurchaseDate ?? entry.asset?.purchaseDate ?? null;
  ...
  const resolvedDepartment =
    entry.tempDepartment ?? entry.asset?.department ?? null;
  const resolvedPicEmployee =
    entry.tempPicEmployee ?? entry.asset?.employee ?? null;
  const resolvedPicId =
    entry.tempPicId ?? resolvedPicEmployee?.id ?? entry.asset?.picId;
  const resolvedPicName =
    entry.tempPic ?? resolvedPicEmployee?.name ?? entry.asset?.pic ?? null;
  ...
  const base: Asset = {
    ...
    employee: resolvedPicEmployee
      ? {
          id: resolvedPicEmployee.id,
          employeeId: resolvedPicEmployee.employeeId || undefined,
          name: resolvedPicEmployee.name,
          email: resolvedPicEmployee.email || undefined,
          department: resolvedPicEmployee.department || undefined,
          position: resolvedPicEmployee.position || undefined,
          isActive: resolvedPicEmployee.isActive || undefined,
        }
      : undefined,
    dateCreated:
      entry.asset?.dateCreated || entry.scannedAt || new Date().toISOString(),
  };
...
export default function AssetDetailModal({
  ...
  const [assetSuffix, setAssetSuffix] = useState({
    categoryRoman: "I",
    siteNumber: "01",
  });
    ...
    try {
      ...
      setHistoryError(
        error instanceof Error
          ? error.message
          : "Tidak dapat memuat riwayat aset"
      );
    } finally {
  ...
  const canEdit =
    !forceReadOnly &&
    (user?.role === "ADMIN" ||
      (sessionContext && user?.role === "SO_ASSET_USER"));
        ...
        <div className="mt-2 space-y-2 rounded-lg border border-dashed border-gray-200 bg-gray-50 p-3 text-xs">
          {entry.details.sessionName && (
            <div className="font-medium text-gray-700">
              Session: {entry.details.sessionName}
            </div>
          )}
          ...
          <div>
            ...
            <div>
              {entry.details?.dueDate
                ? new Date(entry.details.dueDate).toLocaleString()
                : "–"}
            </div>
          </div>
        ...
        <div className="mt-2 grid grid-cols-1 gap-2 text-xs text-gray-700 sm:grid-cols-2">
          <div>
            ...
            <div className="font-medium">
              {entry.details?.receivedBy?.name ??
                entry.details?.receivedBy ??
                "–"}
            </div>
          </div>
          <div>
    ...
    const loadAssetData = async () => {
      ...
      if (isSessionContext && sessionContext) {
        try {
          ...
          if (response.ok) {
            ...
            console.warn(
              "Failed to load SO entry, using cached asset data instead."
            );
          }
        } catch (error) {
        ...
        try {
          ...
          if (response.status === 404) {
            alert(
              "Asset not found. The asset may have been deleted by another user."
            );
            onUpdate();
            onOpenChange(false);
            return;
      ...
      if (assetData.notes) {
        try {
          ...
          const fields = Object.entries(additionalInfo).map(
            ([name, value], index) => ({
      ...
      const [prefix = "FA001", categoryPart = "I", sitePart = "01"] = (
        assetData.noAsset || ""
      ).split("/");
  ...
  const fetchMasterData = async () => {
    ...
    try {
      const [sitesRes, categoriesRes, departmentsRes, picsRes] =
        await Promise.all([
          fetch("/api/sites"),
  ...
  const generateAssetSuffix = async (categoryId?: string, siteId?: string) => {
    try {
      ...
      const response = await fetch(
        `/api/assets/generate-number?${params.toString()}`
      );
      const data = await response.json();
      setAssetSuffix({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const validAdditionalFields = additionalFields.filter(
      ...
      if (isSessionContext && sessionContext) {
        ...
        const sessionPayload = {
          ...
          tempNoAsset: combinedAssetNumber,
          tempSerialNo: sanitizeString(payload.serialNo),
          tempPic: isSessionContext
            ? sanitizeString(formData.employee?.name || formData.pic || null)
            : sanitizeString(formData.employee?.name || formData.pic || null),
          tempPicId: isSessionContext
            ? (formData.employee?.id || formData.picId || null)
            : (formData.employee?.id || formData.picId || null),
      ...
      if (
        !isSessionContext &&
        (errorMessage.includes("Asset not found") ||
        ...
        alert(
          `Failed to update asset: ${errorMessage}. Please check if the selected items still exist.`
        );
      } else {
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-2xl w-[95vw] sm:w-full max-h-[90vh] overflow-y-auto mx-auto">
          <DialogHeader className="pb-4 sm:pb-6">
            ...
            {isSessionContext && (
              <p className="text-xs text-primary mt-1">
                Perubahan disimpan pada sesi ini dan baru akan diterapkan ke
                master asset setelah sesi selesai.
              </p>
            )}
          </DialogHeader>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div className="sm:col-span-2">
              ...
              {isEditing && (
                <p className="text-xs text-gray-500 mt-1">
                  Upload a square image (max 50KB) for best results.
                </p>
              )}
              {isEditing ? (
                <div className="mt-2">
                ...
                <div className="mt-2 flex justify-center">
                  {formData.imageUrl ? (
                    <div className="relative">
                      <div className="relative w-64 h-64 sm:w-72 sm:h-72 rounded-xl border-2 border-surface-border bg-gray-50 overflow-hidden shadow-lg">
                        <Image
            ...
            <div className="sm:col-span-2">
              <Label htmlFor="noAsset" className="text-sm font-medium">
                No Asset
              </Label>
              {isEditing ? (
                <>
                  <div className="flex items-center space-x-2 mt-1">
                    <div className="w-28 flex-shrink-0">
                      <Input
                        ...
                        value={assetPrefix}
                        onChange={(e) =>
                          handleAssetNumberChange(e.target.value)
                        }
                        placeholder="FA040.1"
            ...
            <div>
              ...
              {isEditing ? (
                ...
                >
                  ...
                  <SelectContent
                    className="max-h-60 z-[60] overflow-y-auto"
                    position="popper"
                    sideOffset={4}
                  ...
                  >
                    ...
                    <SelectItem value="Active" className="text-sm text-black">
                      Active
                    </SelectItem>
                    <SelectItem value="Broken" className="text-sm text-black">
                      Broken
                    </SelectItem>
                    <SelectItem
                      value="Lost/Missing"
                    ...
                    <SelectItem value="Sell" className="text-sm text-black">
                      Sell
                    </SelectItem>
                  </SelectContent>
                </Select>
            ...
            <div>
              ...
              <Input
                ...
                disabled={!isEditing}
                className={`mt-1 text-sm ${
                  isEditing ? "bg-white text-foreground" : ""
                }`}
                style={
                  !isEditing
                    ? {
                ...
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger className="mt-1 text-sm bg-white text-foreground text-black">
                      <SelectValue
                        placeholder={
                          pics.length === 0 ? "No PIC available" : "Select PIC"
                        }
                      />
                    </SelectTrigger>
                    <SelectContent
                    ...
                    >
                      ...
                      {pics.length > 0 ? (
                        pics.map((pic) => {
                          ...
                          const showPosition =
                            positionLabel &&
                            positionLabel.toLowerCase() !== "test";
            ...
            <div className="sm:col-span-2">
              <Label htmlFor="dateCreated" className="text-sm font-medium">
                Date Created
              </Label>
              <Input
                id="dateCreated"
          ...
          {isSessionContext && (
            <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50/80 p-4">
              <label className="flex items-start gap-3 text-sm text-text-muted">
                ...
                />
                <span>
                  Tandai aset ini sebagai{" "}
                  <span className="font-semibold text-foreground">Pending</span>{" "}
                  bila tag fisik salah, kondisi rusak, atau data tidak sinkron.
                </span>
              </label>
              {entryCrucial &&
                (isEditing ? (
                  <Textarea
                    ...
                    onChange={(event) =>
                      setEntryCrucialNotes(event.target.value)
                    }
                      ...
                      <AlertDialogContent className="mx-4 max-w-sm">
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription className="text-sm">
                            ...
                            delete the asset "{formData.name}" (
                            {formData.noAsset}) from the database.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
