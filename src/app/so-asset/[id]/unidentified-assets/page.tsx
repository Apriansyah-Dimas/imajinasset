...
export default function UnidentifiedAssetsPage() {
  ...
  const fetchUnidentifiedAssets = async () => {
    try {
      const response = await fetch(`/api/so-sessions/${params.id}/unidentified-assets`)
      if (response.ok) {
  ...
  const filteredUnidentifiedAssets = data?.missingAssets.filter(asset =>
    ...
    return (
      <div className="container mx-auto py-8">
        <Alert>
          ...
          <AlertDescription>Failed to load unidentified assets data</AlertDescription>
        </Alert>
      </div>
    )
  }
  ...
  return (
    <div className="container mx-auto py-3 px-1.5 sm:py-6 sm:px-4 max-w-full lg:max-w-7xl w-full overflow-x-hidden" style={{ minWidth: '320px', maxWidth: '100vw' }}>
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 gap-3">
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-lg sm:text-2xl font-bold truncate">Remaining Assets</h1>
            <p className="text-xs sm:text-sm text-muted-foreground truncate">{data.session.name} • {data.session.year}</p>
          </div>
          <Badge variant={data.session.status === 'Active' ? 'default' : 'secondary'} className="flex-shrink-0 text-xs">
            {data.session.status}
          </Badge>
        </div>
      </div>
      ...
      <Card className="mb-3 sm:mb-6">
        <CardContent className="pt-3 sm:pt-6 px-3 sm:px-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              <h3 className="font-semibold mb-1 text-sm sm:text-base">Progress</h3>
              <p className="text-xs sm:text-sm text-muted-foreground">
                {data.statistics.scannedAssets} of {data.statistics.totalAssets} scanned
              </p>
            </div>
            <div className="text-center sm:text-right">
              <div className="text-lg sm:text-2xl font-bold">{data.statistics.completionPercentage}%</div>
              <div className="text-xs sm:text-sm text-muted-foreground">Complete</div>
            </div>
          </div>
          <Progress value={data.statistics.completionPercentage} className="mt-2 sm:mt-4" />
        </CardContent>
      </Card>
          ...
          <Card className="w-full overflow-hidden min-w-0">
            <CardHeader className="pb-2 sm:pb-3 px-2 sm:px-6">
              <div className="flex items-center justify-between min-w-0">
                <CardTitle className="text-sm sm:text-lg truncate">Assets to Scan</CardTitle>
              ...
              {filteredUnidentifiedAssets.length === 0 ? (
                <div className="text-center py-4 sm:py-8 text-muted-foreground">
                  ...
                  <p className="text-xs sm:text-sm">
                    {searchQuery ? 'Try adjusting search' : 'Great job! All assets scanned.'}
                  </p>
                </div>
              ) : (
                    ...
                    >
                      <div className="flex-1 min-w-0 overflow-hidden w-full">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 mb-1">
                          <span className="font-medium text-xs truncate flex-shrink-0 max-w-[120px]">{asset.noAsset}</span>
                          <div className="flex gap-1 flex-wrap">
                            <Badge variant="outline" className="text-xs px-1 py-0 leading-none flex-shrink-0">
                              {asset.status.length > 6 ? asset.status.substring(0, 6) + '..' : asset.status}
                            </Badge>
                            <Badge variant="secondary" className="text-xs px-1 py-0 leading-none flex-shrink-0">
                              M
                            </Badge>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1 text-xs text-muted-foreground">
                          <span className="flex-shrink-0">S: {(asset.site?.name || 'N/A').substring(0, 6)}</span>
                          <span className="flex-shrink-0">P: {(asset.pic || 'N/A').substring(0, 4)}</span>
                          <span className="flex-shrink-0">C: {(asset.category?.name || 'N/A').substring(0, 4)}</span>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <Card>
            ...
            <CardContent className="space-y-2 sm:space-y-4 px-3 sm:px-6">
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div>
                  <div className="text-base sm:text-xl font-bold">{data.statistics.missingAssets}</div>
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div>
                  <div className="text-base sm:text-xl font-bold">{data.statistics.scannedAssets}</div>
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div>
                  <div className="text-base sm:text-xl font-bold">{data.statistics.totalAssets}</div>
          ...
          <Card>
            <CardContent className="pt-2 sm:pt-4 px-3 sm:px-6">
              <div className="space-y-1 sm:space-y-2">
                <Link href={`/so-asset/${params.id}/scan`}>
                  <Button variant="outline" className="w-full text-xs sm:text-sm px-2 py-1">
